create extension vector;
    create table categories(id uuid primary key, name text, parent_id uuid, slug text, is_active boolean default true);
    ...
    returns table(id uuid, score double precision) language sql stable as $$
      select id, 0.9::double precision from products where name = 'Pastèque' $$;
